    ...
    def execute_query(self, **kwargs):
        if self.pagetoken is not None:
            return (
                ...
                .list(
                    customerId="my_customer", maxResults=1000, pageToken=self.pagetoken
                )
                .execute()
            )
        else:
            return (
                self.service.chromeosdevices()
                .list(customerId="my_customer", maxResults=1000)
                .execute()
            )
...
class authService:
    ...
    def getService(self, *args, **kwargs):
        ...
        return googleapiclient.discovery.build(
            args[0], args[1], credentials=credentials
        )
...
class gSheet(clientAuth):
        ...
        if self.id:
            try:
                ...
                self.data = (
                    ...
                    .values()
                    .get(spreadsheetId=self.id, range=sheet)
                    .execute()
                )
    ...
    def pushData(self, sheet):
        ...
        result = (
            ...
            .values()
            .update(
                spreadsheetId=self.id, range=sheet, valueInputOption="RAW", body=body
            )
            .execute()
        )
...
class TurmaClassroom:
    def __init__(self, turma_id_gsuite, json_file, pessoa):
        ...
        self.service = authService(self.escopos, self.json, pessoa).getService(
            "classroom", "v1"
        )
        self.name = ""
        self.section = ""
    ...
    def carregar_turma(self, turma_pronta=None):
        try:
            ...
            if turma_pronta is None:
                self.turma_gsuite = (
                    self.service.courses().get(id=self.turma_id).execute()
                )
            else:
    ...
    def filtrar(self, criterio):
        ...
        for atv in self.atividades:
            topico = (
                self.service.courses()
                .topics()
                .get(courseId=self.turma_id, id=atv.get("topicId"))
                .execute()
            )
    ...
    def copiar_avisos_mural(self, id_turma, professor_email, logger=None):
        ...
        if not self.avisos:
            ...
            servico_temporario = self.inpersona_professor(
                professor_email
            )  # * INICIA O SERVIÇO IMPERSONADO COMO A ESCOLA
            if servico_temporario:  # * SE DEU CERTO CONTINUA
                for att in self.avisos:
                    ...
                    novo_aviso["individualStudentsOptions"] = att.get(
                    ...
                    try:
                        servico_temporario.courses().announcements().create(
                            courseId=id_turma, body=novo_aviso
                        ).execute()
    ...
    def copiar_atividades_para_turma(self, id_turma, professor_email, logger):
        ...
        if servico_temporario:  # * SE DEU CERTO CONTINUA
            ...
            for atividade in self.atividades:
                ...
                nova_atividade["submissionModificationMode"] = atividade.get(
                ...
                self.copiar_atividade(
                    id_turma, servico_temporario, nova_atividade, logger
                )
    ...
    def obter_topico_nome(self, topico_id):
        ...
        topico = (
            self.service.courses()
            .topics()
            .get(courseId=self.turma_id, id=topico_id)
            .execute()
        )
    ...
    def inpersona_professor(self, email_prof):
        ...
        try:
            return authService(self.escopos, self.json, email_prof).getService(
                "classroom", "v1"
            )
        except:
            return False

    def cria_topico_se_nao_existe(
        self, servico_temporario, turma_temporaria_id, topico_nome
    ):
        try:
            nome = topico_nome
            ok = self.topico_ja_existe(
                servico_temporario, turma_temporaria_id, topico_nome
            )  # verifica se já tem tópico
            if ok.get("pode_criar"):  # se não existir ainda, então cria
                novo_topico = (
                    servico_temporario.courses()
                    .topics()
                    .create(courseId=turma_temporaria_id, body={"name": topico_nome})
                    .execute()
                )
    ...
    def topico_ja_existe(self, servico_temporario, turma_temporaria_id, topico_nome):
        ...
        ja_existe = (
            servico_temporario.courses()
            .topics()
            .list(courseId=turma_temporaria_id)
            .execute()
        )
    ...
    def deletar_atividades(self, turma_id, professor_email, criterios):
        ...
        if servico_temporario:  # * SE DEU CERTO CONTINUA
            ...
            if (
                ...
                and any(cc in turma.get("name") for cc in criterios_map_nivel)
                ...
                atividades = (
                    servico_temporario.courses()
                    .courseWork()
                    .list(courseId=turma_id)
                    .execute()
                    .get("courseWork", False)
                )
                if atividades:
                    atividades = [
                        atv
                        for atv in atividades
                        if "Avaliação diagnóstica" == atv.get("title")
                    ]
                    if len(atividades):
                        ...
                        for atv in atividades:
                            try:
                                servico_temporario.courses().courseWork().delete(
                                    courseId=turma_id, id=atv.get("id")
                                ).execute()
                                msg = "Atividade {} apagad com sucesso na turma {}".format(
                                    atv.get("title"), turma_id
                                )
                                print(msg)
                            except Exception as e:
                                print(
                                    "Falha ao apagar a atividade na turma: {}".format(
                                        turma_id
                                    )
                                )
    ...
    def copiar_atividade(self, turma_temporaria_id, servico_temporario, body, logger):
        try:
            ...
            servico_temporario.courses().courseWork().create(
                courseId=turma_temporaria_id, body=body
            ).execute()
            logger.add_row_csv(
                [
                    "Sucesso",
